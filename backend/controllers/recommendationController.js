...
/**
 * GET /api/recommendations/my    (student only)
 * Recommend courses based on student's enrollment history.
 */
export const getMyCourseRecommendations = async (req, res) => {
  try {
    const studentId = req.user.id || req.user._id;

    // 1) All courses this student is enrolled in
    const myCourses = await Course.find({
      enrolledStudents: studentId,
    }).select("category title");

    // Build category frequency: { "Algorithms": 3, "DSA": 1, ... }
