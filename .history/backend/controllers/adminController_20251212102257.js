...
export const getAllContentReports = async (req, res) => {
  try {
    ...
    const formatted = reports.map((r) => {
      ...
      } else if (r.targetType === "question" && r.question) {
        targetLabel = `Question: ${r.question.title}`;
      } else if (r.targetType === "answer" && r.answer) {
        targetLabel = `Answer: ${r.answer.text?.slice(0, 60)}...`;
      } else if (r.targetType === "user" && r.user) {
        targetLabel = `User: ${r.user.name} (${r.user.email})`;
      }
