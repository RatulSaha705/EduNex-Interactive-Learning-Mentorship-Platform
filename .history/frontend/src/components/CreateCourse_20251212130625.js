...
export default function CreateCourse() {
  ...
  // Redirect guard: only instructors can create courses
  ...
  useEffect(() => {
    const fetchInstructorCourses = async () => {
      ...
      try {
        ...
        // We use instructor name filter (backend supports ?instructor=...)
        ...
        // For a *new* course, we can use all non-archived courses as prerequisites
    ...
    try {
      ...
      await axios.post(
        "http://localhost:5000/api/courses",
        {
          ...
          prerequisites: form.prerequisites, // 🆕 send to backend
        },
        {
          headers: { Authorization: `Bearer ${auth.token}` },
        }
